...
public class SparkCalculatePropertiesFromMedialSurface {
	...
	public static final List<BlockInformation> calculateDistanceTransformAtMedialSurface(final JavaSparkContext sc,
		...
		SparkCosemHelper.createDatasetUsingTemplateDataset(n5Path, datasetName, n5OutputPath, outputDatasetName, DataType.FLOAT32);
			...
			for (int j = 0; j < updatedBlockInformationList.size(); j++) {
				...
				if (gridDistance < numberOfBlocksToExpandBy) {
					if (paddingForMedialSurfaceOne > maxPaddingForBlock[j]) {
						// in case it goes beyond the neighboring, box, need to expand it by the box dimension
	...
	public static final Map<List<Integer>, Long> projectCurvatureToSurface(final JavaSparkContext sc,
		...
		SparkCosemHelper.createDatasetUsingTemplateDataset(n5Path, datasetName, n5OutputPath, outputDatasetName,
				DataType.UINT8);
		...
		JavaRDD<Map<List<Integer>, Long>> javaRDDHistogramMaps = rdd.map(blockInformation -> {
			...
			{ // Necessary? scoping to limit memory
				...
				RandomAccess<FloatType> distanceTransformRA = SparkCosemHelper.getOffsetIntervalExtendZeroRA(n5OutputPath,
						datasetName + "_medialSurfaceDistanceTransform", paddedOffset, paddedDimension);
	...
	public static final HistogramMaps calculateSheetnessAndVolume(final JavaSparkContext sc, final String datasetName,
		...
		JavaRDD<HistogramMaps> javaRDDHistogramMaps = rdd.map(blockInformation -> {
			...
			// Get corresponding medial surface and sheetness
			RandomAccess<UnsignedByteType> sheetnessRA = SparkCosemHelper.getOffsetIntervalExtendZeroRA(n5OutputPath,
	...
	public static final void main(final String... args) throws IOException, InterruptedException, ExecutionException {
		...
		for (String organelle : options.getInputN5DatasetName().split(",")) {
			...
			if (options.getCalculateAreaAndVolumeFromExistingDataset()) {
				HistogramMaps histogramMaps = calculateSheetnessAndVolume(sc, organelle, options.getOutputN5Path(),
						blockInformationList);
				writeData(histogramMaps, options.getOutputDirectory(), organelle, false);
			} else {
				blockInformationList = calculateDistanceTransformAtMedialSurface(sc, options.getInputN5Path(),
						organelle, options.getOutputN5Path(), blockInformationList);
				Map<List<Integer>, Long> sheetnessAndThicknessHistogram = projectCurvatureToSurface(sc,
						options.getInputN5Path(), organelle, options.getOutputN5Path(), blockInformationList);
